...
class DaskSVM:
    ...
    def _fit_with_split(self, x, y):
        ...
        for w, partition_data in partitions:
            indexes, part = tuple(zip(*partition_data))

            partitioned_x.append(part)
            ...
            partitioned_y.append(self._client.compute(y_blocks, workers=w))

            size = sum(x.blocks[i].shape[0] for i in indexes)
            ids.append(self._client.submit(_random_ids, size, workers=w))

            workers.append(w)
        
        for _ in range(NUMBER_OF_CSVM_ITERATIONS - 1):
            self._do_iteration_with_split(partitioned_x, partitioned_y, ids, workers)
        self._do_iteration_with_split(partitioned_x, partitioned_y, ids, workers, last_iteration=True)

    def predict(self, x):
        return self._clf.predict(x)
    
    def _do_iteration_with_split(self, parted_x, parted_y, construct_ids, workers, last_iteration=False):
        q = []
        ...
        for parted_x, parted_y, id_block, w in zip(parted_x, parted_y, construct_ids, workers):
            _train_data = self._client.submit(_train_partition, parted_x, parted_y, id_block, extras, workers=w)
            q.append(_train_data)

        while len(q) > 24:
            data = q[:24]
            del q[:24]

            new_elem = self._client.submit(_train_reduction, data)
            q.append(new_elem)
